...
def scrape_gexa_txu() -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Failed to fetch Gexa/TXU page: {e}")
        return []
...
def scrape_direct_energy() -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Failed to fetch Direct Energy page: {e}")
        return []
    ...
    for table in soup.find_all("table"):
        for row in table.find_all("tr"):
            try:
                ...
                logger.warning(f"Skipping a row in Direct Energy scraper due to parsing error: {e}")
                continue
    return plans
...
def scrape_reliant() -> List[Dict]:
    ...
    term and rate at the 1,000Â kWh tier.  Note that some plans like
    ...
    try:
        ...
        logger.error(f"Failed to fetch Reliant Energy page: {e}")
        return []
...
def scrape_txu() -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Failed to fetch TXU Energy page: {e}")
        return []
...
def scrape_all() -> List[Dict]:
    ...
    for scraper in (scrape_gexa_txu, scrape_direct_energy, scrape_reliant, scrape_txu):
        try:
            ...
            logger.error(f"Scraper {scraper.__name__} failed: {exc}")
